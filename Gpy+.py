...
channel = "@ICEROBOTS" # Channel username without @
logs = "ICEROBOTS" # Channel username with @
token = "1846248575:AAESnYnWZC2cnhMGL9ammmUqL5BoeLoXTm4" # Bot token
...
""")
    ...
    if chat_type=='private':    
        if statusch in ['creator', 'administrator', 'member']:
            if text[:6] == '/start':
                bot.sendMessage(chat_id, """» سلام ، به ربات مدیریت گروه رایگان *ایس تیم* خوش اومدی!
...
############################################################################################
    if content_type=='text' and from_id in sudo_users:
        ...
        if msg['text']=='همکاران':
            b = '» لیست همکاران فعال ربات مدیریت گروه ایس تیم :\n\n'
            for i in sudo_users:
                a = bot.getChat(i)
                c = a['first_name']
                d = a['id']
...
def inlines(msg):
    ...
    if chat_type=='private':
        ...
        if query_data=='pvmain':
            bot.editMessageText((chat_id, message_id), """» سلام ، به ربات مدیریت گروه رایگان *ایس تیم* خوش اومدی!
» این سرویس ، جدیدترین ربات مدیریت گروه با امکانات متنوع میباشد و از جمله امکانات مطرح این ربات میتوان به بیش از ۲۰ قفل که هرکدام دارای قابلیت شخصی سازی هستند ، اشاره کرد.
» کافیست ربات در گروه خودتون عضو کنید ، عبارت ( نصب ) یا ( add ) رو درگروه بفرستید و شاهد قدرت این ربات فوق‌العاده باشید...
...
» تعداد پیام‌ها : {}""".format(
    ...
    message_id
        ...
        bot.editMessageText((chat_id, message_id), """*راهنمای سودوهای ربات مدیریت گروه رایگان ایس تیم :*
...
`خارج شدن ربات از گروه مورد نظر`""" , reply_markup=back , parse_mode='markdown')
        ...
        bot.editMessageText((chat_id, message_id), """*راهنمای تنظیمی ربات مدیریت گروه رایگان ایس تیم :*
...
مثال : حذف فیلتر بای""" , reply_markup=back , parse_mode='markdown')
        ...
        bot.editMessageText((chat_id, message_id), """*راهنمای لیستی ربات مدیریت گروه رایگان ایس تیم :*
...
`دریافت لیست کلی اطلاعات گروه`""" , reply_markup=back , parse_mode='markdown')
        ...
        bot.editMessageText((chat_id, message_id), """*راهنمای مدیریتی ربات مدیریت گروه رایگان ایس تیم :*
...
`پاکسازی پیام‌های گروه به تعداد ذکر شده ( بین ۱ تا ۱۵۰ پیام )`""" , reply_markup=back , parse_mode='markdown')
        ...
        bot.editMessageText((chat_id, message_id), """*» راهنمای قفل‌های ربات مدیریت گروه رایگان ایس تیم :*
...
_لینک | فوروارد | پاسخ | تگ | هشتگ |‌ انگلیسی | فارسی | فحش | شکلک | ویرایش | منشن | ورود ربات_""" , reply_markup=back , parse_mode='markdown')
        ...
        bot.editMessageText((chat_id, message_id), """*راهنمای سرگرمی ربات مدیریت گروه رایگان ایس تیم‌ :*
